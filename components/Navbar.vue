    ...
    <Disclosure as="nav" class="bg-white" v-slot="{ open }">
          ...
          <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-between">
            ...
            <div class="hidden sm:ml-6 sm:block">
              <div class="flex space-x-4">
                <NuxtLink v-for="item in navigation" :key="item.name" :to="item.href" :class="[item.current ? ' text-gray-800' : 'text-gray-800 hover:bg-gray-300 hover:text-grey', 'rounded-md px-3 py-2 text-sm font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</NuxtLink>
              </div>
            </div>
          </div>
      ...
      <DisclosurePanel class="sm:hidden">
        <div class="space-y-1 px-2 pb-3 pt-2">
          <DisclosureButton v-for="item in navigation" :key="item.name" as="a" :href="item.href" :class="[item.current ? ' text-gray-800' : 'bg-gray-200 hover:text-gray-800', 'block rounded-md px-3 py-2 text-base font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</DisclosureButton>
        </div>
      </DisclosurePanel>
    </Disclosure>
</template>
...
const navigation = [
  ...
  { name: 'Programme', href: '/programme', current: false },
...
watchEffect(() => {
  navigation.forEach(item => {
    item.current = item.href === currentPath.value
  })
})
</script>
